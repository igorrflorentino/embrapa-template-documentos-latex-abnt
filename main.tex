...
% Tipo de documento — a escolha estrutural fundamental:
%   publicacao  - Publicação técnico-científica em formato ABNT completo
%                 (com banca, ficha catalográfica, resumo/abstract etc.).
%   corporativo - Relatório corporativo/empresarial (modo não acadêmico:
%                 usa "Sumário executivo" no lugar de resumo/abstract e
%                 dispensa banca, folha de aprovação e ficha catalográfica).
%
% O tipo e o subtipo abaixo usam \providecommand para que os arquivos de
% demonstração (exemplo-*.tex) possam sobrescrevê-los via \def antes do
% \input{main.tex} — veja ./gerar-exemplos.sh. Na compilação normal, valem
% os padrões definidos aqui.

\providecommand{\TipoDoc}{publicacao}
\tipodocumento{\TipoDoc}

% Subtipo (relevante no modo 'publicacao') — define o rótulo da capa e a
% frase do preâmbulo, conforme a série de publicação da Embrapa:
...
\providecommand{\SubTipo}{relatorio}
\subtipo{\SubTipo}

% Série não prevista? Em vez de \subtipo, defina os dois campos diretamente
% (eles são o que o \subtipo preenche por baixo). Ex.: Circular Técnica:
