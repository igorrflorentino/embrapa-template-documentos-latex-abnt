...
- **Tipos de documento (`\tipodocumento` + `\subtipo`)**: `\tipodocumento{publicacao|corporativo}` é a escolha estrutural; no modo `publicacao`, `\subtipo{relatorio|boletim|comunicado|documento}` define o rótulo da capa e a frase do preâmbulo. Implementação em `lib/embrapatex.sty`: o booleano `embrapatex@ehcorporativo` + `\ifcorporativo` (modo), os booleanos `embrapatex@sub<serie>` (subtipo), as macros `\imprimircapa<serie>`/`\preambulo<serie>` e o roteamento em `\imprimircapa`/`\preambulo`. O modo `corporativo` adiciona `\imprimircapacorporativo`, `\preambulocorporativo`, `\imprimirsumarioexecutivo` e ignora o `\subtipo`. Para adicionar uma nova série, replique o par capa+preâmbulo e um ramo no `\subtipo`.
