...
- **`academico`** (padrão) — publicação técnico-científica em formato ABNT completo (banca, ficha catalográfica, resumo/abstract etc.). A variação específica é dada por `\subtipo{...}` (a série de publicação da Embrapa): `relatorio` (padrão), `boletim`, `comunicado` ou `documento`. O `\subtipo` afeta apenas o **rótulo da capa** e a **frase do preâmbulo** da folha de rosto — a estrutura é a mesma.
