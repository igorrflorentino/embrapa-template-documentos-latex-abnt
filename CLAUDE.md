...
**Há dois tipos de documento fundamentais**, escolhidos por `\tipodocumento{...}` no `main.tex`:

- **`publicacao`** (padrão) — publicação técnico-científica em formato ABNT completo (banca, ficha catalográfica, resumo/abstract etc.). A variação específica é dada por `\subtipo{...}` (a série de publicação da Embrapa): `relatorio` (padrão), `boletim`, `comunicado` ou `documento`. O `\subtipo` afeta apenas o **rótulo da capa** e a **frase do preâmbulo** da folha de rosto — a estrutura é a mesma.
- **`corporativo`** — relatórios empresariais/não acadêmicos (ex.: análise exploratória de dados de uma commodity). Ativa o condicional `\ifcorporativo` (em `lib/embrapatex.sty`), que troca o conjunto de pré-textuais: usa um **Sumário executivo** (`elementos-pre-textuais/sumario-executivo.tex`) no lugar de resumo/abstract e dispensa banca, folha de aprovação e ficha catalográfica. Os metadados acadêmicos podem permanecer preenchidos — são ignorados nesse modo.

A cópia de trabalho atual está configurada como `publicacao` + `\subtipo{relatorio}` (relatório final) para um probatório.

Houve **ruptura limpa** de API: os valores antigos `\tipodocumento{relatorio|boletim|comunicado|documento}` não são mais aceitos — agora são `\subtipo`. Usá-los emite um `\PackageWarning` e assume `publicacao`. Há ainda o modificador ortogonal `\ehqualificacao{sim}`, que marca proposta de qualificação e suprime vários pré-textuais (combinável com `publicacao`).
...
- **Tipos de documento (`\tipodocumento` + `\subtipo`)**: `\tipodocumento{publicacao|corporativo}` é a escolha estrutural (booleano `embrapatex@ehcorporativo` + `\ifcorporativo`). O **subtipo é orientado a dados**, não a código: `\subtipo{relatorio|boletim|comunicado|documento}` é uma tabela que apenas preenche duas variáveis — `\subtitulodacapa` (subtítulo da capa; vazio = sem subtítulo) e `\naturezadapublicacao` (frase do preâmbulo). Há **uma única** macro de capa (`\imprimircapapublicacao`, que imprime o subtítulo só se não vazio) e o `\preambulo` lê `\naturezadapublicacao`. O modo `corporativo` tem `\preambulocorporativo` e `\imprimirsumarioexecutivo` próprios, fixa o subtítulo "Relatório" no roteamento de `\imprimircapa` e ignora o `\subtipo`. **Para uma série nova**: adicione um ramo de 2 linhas em `\subtipo` **ou** simplesmente defina `\subtitulodacapa{...}`/`\naturezadapublicacao{...}` no `main.tex` (sem tocar no `.sty`).
